...
__global__ void
    ...
    kernel_moe_gemm_blockscale(typename GridwiseGemm::Argument karg)
{
...
#endif
    // __attribute__((amdgpu_waves_per_eu(1, 1)))
    kernel_moe_gemm_blockscale_2lds(typename GridwiseGemm::Argument karg)
{
#if(!defined(__HIP_DEVICE_COMPILE__) || defined(__gfx9__))
    __shared__ char p_shared[GridwiseGemm::GetSharedMemoryNumberOfByte()];
...
struct GridwiseMoeGemm_BlockScale
{
    ...
    struct SplitKBatchOffset
    {
        __device__ SplitKBatchOffset(Argument& karg)
        {
            if constexpr(is_same_v<tensor_layout::gemm::RowMajor, ALayout>)
            {
                a_k_split_offset =  blockIdx.z / APackedSize;
            }
            else if constexpr(is_same_v<tensor_layout::gemm::ColumnMajor, ALayout>)
            {
    ...
    {
        ignore                           = b_element_op;
        static_assert(IsInputGemm == false, "Only support Moe Gemm2.\n");
        ...
        const auto a_scale_grid_desc_am_ak = make_naive_tensor_descriptor(
            make_tuple(math::integer_divide_ceil(IsInputGemm ? problem.NumTokens : problem.NumTokens * problem.TopK, ScaleBlockM),
        ...
        const auto b_scale_grid_buf = make_dynamic_buffer<AddressSpaceEnum::Global>(
            p_b_scale_grid + expert_id * expert_scale_stride / BPackedSize, b_scale_grid_desc_bn_ak.GetElementSpaceSize());
        // A matrix in LDS memory, dst of blockwise copy
        constexpr auto a_block_desc_ak0_m_ak1 = GetABlockDescriptor_AK0PerBlock_MPerBlock_AK1();
